...
golf.loadAll = function() {
    timer.createInterval('golf.timer', golf.timer, 1000);
};
...
golf.drawGolfHud = function() {
    if (golfplaying) {
        ...
        if (golfhole != 0 ) {
            //Math.ceil()
            let distance = Math.ceil(methods.distanceToPos(mygolfball.position, new mp.Vector3(holes[golfhole].x2,holes[golfhole].y2,holes[golfhole].z2)));
            ui.drawText("~s~" + golfstrokes + "~r~ - ~s~" + totalgolfstrokes + "~r~ - ~s~" + clubname + "~r~ - ~s~" + distance + " m", 0.5,0.93, 1, 255,255,255,255,1, 1)
        }
    }
};
...
golf.timer = function() {
    if (golfplaying) {
        let dist = methods.distanceToPos(mp.players.local.position, new mp.Vector3(-1332.7823486328,128.18229675293,57.032329559326));
        if (ballstate == 1) {
            golfhole = golfhole + 1;
            if (golfhole == 10)
                golf.endgame();
...
golf.createBall = function(x, y, z) {
    ...
    mygolfball.setHeading(mp.players.local.getRotation(0).z);
};
...
golf.MoveToBall = function() {
    if (golfstate == 1) {
        ...
        let playerLoc = mp.players.local.position;
        let distance = methods.distanceToPos(playerLoc, ballLoc);
        if (distance < 50.0) {
...
golf.dropShot = function() {
    if (doingdrop) {
        let distance = methods.distanceToPos(mygolfball.position, mp.players.local.position);
        let distanceHole = methods.distanceToPos(mp.players.local.position, new mp.Vector3(holes[golfhole].x2,holes[golfhole].y2,holes[golfhole].z2));
